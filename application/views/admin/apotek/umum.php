																	...
																	<div class="modal-content">
																		...
																		<div class="modal-body">
																			<?php foreach ($poli as $p) : ?>
																				<?php if ($p['poli_id'] == 1) : ?>
																					<div class="form-group">
																						<label class="control-label" for="select-asal-poli"><b>Asal Poli</b></label>
																			...
																			<div class="form-group">
																				<label class="control-label"><b>Biaya Racik</b></label>
																				<div class="input-group">
																					<span class="input-group-btn">
																						<button class="btn btn-primary btn-icon" type="button"><i class="icon-medicine"></i></button>
																					</span>
																					<input type="text" class="form-control rupiah" name="biaya_racik" id="input-biaya-racik" onkeyup="hitung_kembalian()">
																				</div>
																			</div>
																			
																			<div class="form-group">
																				<label class="control-label"><b>Embalase </b></label>
																				<div class="input-group">
																					<span class="input-group-btn">
																						<button class="btn btn-primary btn-icon" type="button"><i class="icon-medicine"></i></button>
																					</span>
																					<input type="text" class="form-control rupiah" name="embalase" id="input-embalase" onkeyup="hitung_kembalian()" >
																				</div>
																			</div>
																			...
																			<div class="form-group">
																				...
																				<div class="input-group">
																					...
																					<input type="text" class="form-control rupiah" name="dibayar" id="input-dibayar" onkeyup="hitung_kembalian()">
																				</div>
																			</div>
				...
				success: (res) => {
					...
					if (res.status == false) {
						...
						for (const item of res.data) {
							...
							if (item.stok < 0) {
								span_stok = `<span class="label label-danger">${item.stok}</span>`;
							} else {
								span_stok = `<span class="label label-success">${item.stok}</span>`;
								cursor = `style="cursor:pointer;" onclick="add_row('${item.id_barang}', '${item.nama_barang}', '${item.kode_barang}', '${item.harga_jual}', '${item.laba}', '${item.stok}', '${item.id_jenis_barang}')"`; 
							}

							let blinkingRedDot = '';
							if (item.stok < 5) {
								blinkingRedDot = '<span style="padding-left: 5px; font-family: Roboto, sans-serif; font-weight: bold; color: white;" class="bintik-merah">Stok Hampir Habis</span>';
							} else if (item.stok <= 0) {
								blinkingRedDot = '<span style="padding-left: 5px; font-family: Roboto, sans-serif; font-weight: bold; color: white;" class="bintik-merah">Stok Sudah Habis</span>';
							}

							// Calculate days remaining until expiration
							const expirationDate = new Date(item.tanggal_kadaluarsa);
							const today = new Date();
							const daysRemaining = Math.floor((expirationDate - today) / (1000 * 60 * 60 * 24));

							let expirationNotification = '';
							if (daysRemaining <= 5 && daysRemaining >= 0) {
								// Show expiration notification
								expirationNotification = `
									<span style="padding-left: 5px; font-family: Roboto, sans-serif; font-weight: bold; color: black;" class="bintik-merah">
										Kadaluarsa dalam ${daysRemaining} hari
									</span>`;
							}
								...
								<div class="item-barang" ${cursor}>
									...
									<div class="detail-barang">
										<div class="nama-barang">
											<span style="margin-left:-20px; padding-top:-20px;">${blinkingRedDot}${expirationNotification}</span>
    ...
    $('#search-barang-kasir-umum').off('keyup').keyup(function () {
        data_barang();
				...
				function data_barang() {
					...
					// 									<div class="nama-barang">
					// 										<h5 class="title">${item.kode_barang} - ${item.nama_barang}</h5>
					// 										<span>Stok : ${span_stok} ${item.stok < 3 ? '<span style="display: inline-block; width: 6px; height: 6px; background-color: red;border-radius: 50%;margin-left: 5px;    @keyframes blink { 0%, 50%, 100% { opacity: 1; } 25%, 75% { opacity: 0;} }" class="bintik-merah"></span>' : ''}</span>
					// 									</div>
					// 									<div class="action-barang">
					// 										<button type="button" class="btn bg-primary-600 btn-icon btn-rounded"><i class="icon-plus3"></i></button>
					// 									</div>
					...
					let hitung_kembalian = () => {
							...
							const nilaiTransaksi = parseFloat($('#input-nilai-transaksi').val()) || 0;
							const nilaiDibayar = parseFloat($('#input-dibayar').val()) || 0; 
							const biayaRacik = parseFloat($('#input-biaya-racik').val()) || 0;
							const biayaEmbalase = parseFloat($('#input-embalase').val()) || 0;

							const tambah_biaya_racik = nilaiTransaksi + biayaRacik;
							const tambah_embalase = tambah_biaya_racik + biayaEmbalase;

							const kembalian = Math.abs(tambah_embalase - nilaiDibayar);
							const btnProsesBayar = $('#btn_proses_bayar');

							btnProsesBayar.prop('disabled', nilaiDibayar < nilaiTransaksi);

							$('.rupiah').mask('000.000.000', {
								reverse: true
							});

							$('#input-kembali').val(convertRupiah(kembalian));
						};

						$('#input-dibayar, #input-biaya-racik, #input-embalase').on('input', hitung_kembalian);
