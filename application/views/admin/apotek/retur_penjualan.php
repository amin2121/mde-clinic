...
<head>
	...
	<script type="text/javascript">
		...
		function get_retur() {
			...
			$.ajax({
				url: '<?php echo base_url(); ?>apotek/Retur_penjualan/get_retur',
				data: {
					search: search
				},
				type: "POST",
				...
				success: function(result) {
					...
					if (result == "" || result == null) {
						...
						for (var i = 0; i < result.length; i++) {
							...
							$tr += '<tr style="cursor:pointer;" onclick="klik_retur(' + result[i].id + ');">' +
								'<td>' + result[i].no_transaksi + '</td>' +
								'<td>' + result[i].nama_pasien + '</td>' +
								'<td style="text-align:center;">Rp.' + convertRupiah(parseInt(result[i].nilai_transaksi)).replace(/\./g, ',') + '</td>'+
				...
				<div class="content">
					...
					<div class="row">
						...
						<div class="col-sm-7">
							...
							<div class="panel panel-flat border-top-success border-top-lg">
								<div class="panel-heading">
									<h6 class="panel-title">Retur penjualan</h6>
								</div>

								<div class="panel-body">
									<input type="text" id="input-index" value="0" hidden="">
									<form action="<?= base_url('apotek/retur_penjualan/transaksi_retur') ?>" method="POST" id="form-checkout">

										<div class="form-group">
											<div class="input-group">
												<span class="input-group-btn" >
													<button class="btn bg-primary btn-sm" onclick="modal_retur(); get_retur();"   type="button" style="margin-bottom:20px;"><i class="fa fa-search position-left"></i> Cari</button>
												</span>
												<input type="text" id="nama_pasien" name="nama_pasien" class="form-control" placeholder="Nama Pasien" readonly>
												<input type="text" id="no_transaksi" name="no_transaksi" class="form-control" placeholder="No transaksi" readonly>
										...
										<div class="table-responsive">
											<table class="table table-striped table-hover">
												...
												<thead class="bg-primary">
													<tr>
														<!-- <th>jumlah yang ingin di kembalikan</th> -->
...
p														<th>Nama Barang</th>
												...
												<tfoot>
													<tr>
														<td colspan="4" class="text-right text-semibold">
															<h2>Total (Rp.)</h2>
														</td>
														<td colspan="2">
																	...
																	<div class="modal-content">
																		...
																		<div class="modal-body">
																			...
																			<div class="form-group">
																				...
																				<!-- <php foreach ($get_dibayar as $key => $bayar): ?> -->
																					<input type="text" class="form-control rupiah" name="dibayar" value="" id="input-dibayar" onkeyup="hitung_kembalian(this)">
																					<!-- <php  endforeach  ?> -->
																				</div>
																			</div>
				...
				<script>
					function klik_retur(id) {
						$('#tutup_data_pasien').click();

						$.ajax({
								url: '<?php echo base_url(); ?>apotek/Retur_penjualan/klik_retur',
								data: {
									id: id
								},
								type: "POST",
							...
							.done(function(data) {
								...
								$.each(data, function(index, row) {
									...
									<tr id="row-${row.id}" class="row-${row.id}">
										...
										<td>
											<input type="hidden" value="${row.id_barang}" name="id_barang[]" id="input-id-barang-${row.id}" class="input-id-barang-${row.id}"/>
											<input type="hidden" value="${parseInt(row.laba)}" name="laba[]" id="input-laba-${row.id}" class="input-laba input-laba-${row.id}" />
											<input type="text" size="5" class="form-control qty-${row.id}" id="qty-${row.id}" name="qty[]" onkeyup="hitung_total_harga_beli(${row.id}, '${row.harga_jual}', '${row.laba}');" value="${row.jumlah_beli}">
										</td>
										<td><span id="nama-barang-${row.id}" name="nama_barang[]" class="nama-barang-${row.id}">${row.nama_barang}</span></td>
										<td><span id="kode-barang-${row.id}" name="kode_barang[]" class="kode-barang-${row.id}">${row.kode_barang}</span></td>
										<td><span class="harga-${row.id}" name="harga_jual[]" id="harga-${row.id}">Rp. ${convertRupiah(parseInt(row.harga_jual))}</span></td>
										<td>
											<span id="text-subtotal-${row.id}" name="subtotal[]" class="text-subtotal-${row.id}" value="${convertRupiah(parseInt(row.subtotal))}">Rp. ${convertRupiah(parseInt(row.harga_jual))}</span>
										</td>
										<td>
								...
								`;
									...
									if (row.stok < 0) {
										// Handle negative stok if needed
									} else {
										if (jumlah_data > 0) {
								...
								// Call data_barang() function here after the loop finishes
								data_barang();
							...
							.always(function() {
								...
								// Panggil callback setelah data berhasil dimuat
					...
					$(window).load((e) => {
						...
						$('#btn_proses_bayar').click(function() {
							...
							$.ajax({
								url: '<?php echo base_url(); ?>apotek/retur_penjualan/transaksi_retur',
								...
								success: function(row) {
									...
									// $('#no_transaksi').val(id_penjualan);
									if (row['status'] == true) {
										var win = window.open('<?php echo base_url(); ?>apotek/Retur_penjualan/cetak_struk/' + id_penjualan, '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');
										window.location.href = '<?php echo base_url(); ?>apotek/Retur_penjualan/';
									} else {
										window.location.href = '<?php echo base_url(); ?>apotek/Retur_penjualan/';
									}
							...
							<h5 class="error-barang">${res.message}</h5>
									...
									for (const item of res.data) {
										...
										if (item.stok < 0) {
											...
											var cursor = `style="cursor:pointer;" onclick="add_row('${item.id}', '${item.nama_barang}', '${item.kode_barang}', '${item.harga_jual}', '${item.laba}', '${item.stok}', '${item.id_jenis_barang}')"`;
										}
					...
					let enter_cari_barang = (e) => {
						if (e.keyCode === 13) {
                            ...
                            `;
											...
											// Call the klik_retur() function with a callback to load other data
											klik_retur(row.id_barang, function() {
												data_barang();
											});
										}
