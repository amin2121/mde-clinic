						...
						<div class="col-sm-12">
							...
							<div class="tabbable">
								<ul class="nav nav-tabs nav-tabs-component">
									<li class="active"><a href="#mutasi" data-toggle="tab">Up Klinik</a></li>
									<li><a href="#tambah-mutasi" data-toggle="tab">Tambah Up Klinik </a></li>
								</ul>
								...
								<div class="tab-content">
											...
											<table class="table table-bordered table-striped" id="table-mutasi">
												<thead>
													<tr class="bg-success">
														...
														<th class="text-center">Total Harga Up klinik</th>
														<th class="text-center">Tanggal Up klinik</th>
									...
									<div class="tab-pane" id="tambah-mutasi">
										<form action="<?= base_url('apotek/up_klinik/tambah_up_klinik_barang') ?>" class="form-horizontal" method="POST" id="form-tambah-mutasi">
													...
													<table class="table table-bordered table-striped" id="table-tambah-mutasi-barang">
														<thead>
															<tr class="bg-success">
																...
																<th class="text-center">Stok di kembalikan</th>
...
<script>
	$(window).load(function() {
		...
		$.ajax({
			url : '<?= base_url('apotek/up_klinik/get_up_klinik_ajax') ?>',
			method : 'POST',
			dataType : 'json',
			beforeSend : () => {
					...
					for(const item of res.data) {
						row += 	`
							<tr>
								...
								<td class="text-center">${item.kode_up_barang }</td>
											...
											<div class="modal-content">
												...
												<div class="modal-body">
													<div class="alert alert-danger no-border">
														<p>Apakah Anda Ingin <span class="text-semibold text-center">Menghapus</span> Penembalian ${item.kode_up_barang} ?</p>
												    </div>
												</div>
												<div class="modal-footer">
													<button class="btn btn-link" data-dismiss="modal"><i class="icon-cross position-left"></i> Keluar</button>
													<a href="<?php echo base_url(); ?>apotek/up_klinik/hapus_up_klinik/${item.id}" class="btn btn-danger"><i class="icon-bin position-left"></i> Hapus</a>
												</div>
											</div>
										</div>
									</div>
	...
	function cari_mutasi_by_tanggal() {
		...
		// $('#popup_load').show();
		$.ajax({
			url : '<?= base_url('apotek/up_klinik/cari_up_klinik_by_tanggal_ajax') ?>',
	...
	function get_barang() {
		...
		$.ajax({
			url : '<?= base_url('apotek/up_klinik/get_barang_stok') ?>',
			method : 'POST',
    ...
    $.ajax({
			url : '<?= base_url('apotek/up_klinik/get_detail_up_klinik') ?>',
      data : {id:id},
			method : 'POST',
			dataType : 'json',
			success : (res) => {
