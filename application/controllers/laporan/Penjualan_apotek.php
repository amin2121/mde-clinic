...
class Penjualan_apotek extends CI_Controller {
    ...
    if ($this->input->get('id_cabang') == 'semua') {
      ...
      $row = 8; // Mulai dari baris kedua
        ...
        $sheet->setCellValue('E7', 'Nilai Transaksi(Rp.)');
        $sheet->setCellValue('F7', 'Nilai Laba(Rp.)');
        ...
        $sheet->getStyle('A7:F7')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
				...
				foreach ($data['result'] as $result) {
					...
					$sheet->setCellValue('D' . $row, wordwrap($result['nama_pasien'], 20, "\n", true));
					$sheet->setCellValue('E' . $row, number_format($result['nilai_transaksi']));
					$sheet->setCellValue('F' . $row, number_format($result['total_laba']));
      ...
      $sheet->getStyle('A7:F' . $row)->applyFromArray($styleArray);
